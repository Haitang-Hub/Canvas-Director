...
function videoPluginResult(result: unknown): VideoGenerationResult {
    ...
    if (result && typeof result === "object") {
        ...
        const rawKeys = Object.keys(record).filter((k) => k !== "code" && k !== "msg" && k !== "error");
        throw new Error(rawKeys.length > 0
            ? `${apiText("scriptNoVideo")}（实际返回字段：${rawKeys.join(", ")}，请检查调用脚本的返回值格式）`
            : apiText("scriptNoVideo"));
    }
    throw new Error(`${apiText("scriptNoVideo")}（脚本实际返回了 ${result === null ? "null" : typeof result}，expected: string URL / Blob / { url } / { blob }）`);
}
...
async function pollGeminiVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        console.log(`[POLL-VIDEO] task=${task.id} done=${state.done} error=${state.error ? "yes" : "no"}`);

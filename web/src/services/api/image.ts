...
// 标准 OpenAI Chat Completions(/v1/chat/completions)流式请求。
// 这是所有 OpenAI 兼容网关(Agnes 等)与官方 OpenAI 通用支持的文本/视觉接口;
// Responses API(/v1/responses)为 OpenAI 专有,多数兼容网关不支持。
async function requestStreamingChatCompletions(config: AiConfig, messages: AiTextMessage[], onDelta?: (text: string) => void, options?: RequestOptions): Promise<string> {
    ...
    if (!response.body) {
        ...
        return chatCompletionsText(payload);
    }
    const reader = response.body.getReader();
    const decoder = new TextDecoder();
    let buffer = "";
    ...
    const consumeBlock = (block: string) => {
        const data = block
            ...
            .trim();
        if (!data || data === "[DONE]") return;
        const event = JSON.parse(data) as ChatCompletionsPayload;
...
function chatCompletionsText(payload: ChatCompletionsPayload) {
    return payload.choices?.map((choice) => choice.message?.content || choice.delta?.content || "").join("") || "";
}

function validateChatCompletionsPayload(payload: ChatCompletionsPayload) {
    if (typeof payload.code === "number" && payload.code !== 0) throw new Error(payload.msg || apiText("requestFailed"));
    if (payload.error?.message) throw new Error(payload.error.message);
}
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    try {
        ...
        // 其余 OpenAI 兼容渠道一律走标准 Chat Completions(官方 OpenAI 与第三方网关通用);
        // 消息体已是 messages + text/image_url 结构,含系统提示词。
        const answer = (await requestStreamingChatCompletions(requestConfig, withSystemMessage(requestConfig, messages) as AiTextMessage[], onDelta, options)) || apiText("noContent");
        if (answer === apiText("noContent")) onDelta(answer);
        return answer;
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("requestFailed")));

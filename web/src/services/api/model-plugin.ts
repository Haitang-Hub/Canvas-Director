...
import { buildApiUrl, withLocalProxy, type AiConfig, type ModelCapability, getDefaultImageScriptIndex, DEFAULT_AGNES_IMAGE_SCRIPT, DEFAULT_AGNES_VIDEO_SCRIPT } from "@/stores/use-config-store";

type RequestOptions = { signal?: AbortSignal; timeoutMs?: number };

/** 图片生成类请求默认超时（ms）。Agnes Image 生成较慢，代理端设为 900s，浏览器端同步匹配。 */
...
function createPluginHttp(config: AiConfig, options?: RequestOptions): PluginHttp {
    const run = async (method: "get" | "post", path: string, body: unknown, opts?: PluginHttpOptions) => {
        ...
        const url = pluginUrl(config, path);
        console.log(`[MODEL] ${method.toUpperCase()} ${url}${body !== undefined ? ` body=${JSON.stringify(body).slice(0, 300)}` : ""}`);
        ...
        console.log(`[MODEL] <- ${response.status} ${JSON.stringify(response.data).slice(0, 300)}`);
...
function createPluginRequest(config: AiConfig, options?: RequestOptions) {
    return async (requestConfig: AxiosRequestConfig & { url: string }) => {
        ...
        console.log(`[MODEL-Raw] ${requestConfig.method?.toUpperCase() ?? "GET"} ${url}${requestConfig.data !== undefined ? ` body=${JSON.stringify(requestConfig.data).slice(0, 300)}` : ""}`);
        const response = await axios.request({ ...requestConfig, url, signal: options?.signal, timeout: options?.timeoutMs ?? DEFAULT_IMAGE_TIMEOUT_MS });
        console.log(`[MODEL-Raw] <- ${response.status} ${JSON.stringify(response.data).slice(0, 300)}`);
        return response.data;
    };
}
...
export async function runModelPlugin<T = unknown>(args: RunPluginArgs): Promise<T> {
    const { config } = args;
    const http = createPluginHttp(config, { signal: args.signal, timeoutMs: args.timeoutMs });
    const request = createPluginRequest(config, { signal: args.signal, timeoutMs: args.timeoutMs });
...
return await generateImage({
  ...
  request,
        ...
        {
            label: i18n.t("modelPlugin.templates.agnes"),
            script: DEFAULT_AGNES_IMAGE_SCRIPT,
        },
    ],
    video: [
        {
...
return await generateVideo({
  ...
  poll,
        ...
        {
            label: i18n.t("modelPlugin.templates.agnesVideo"),
            script: DEFAULT_AGNES_VIDEO_SCRIPT,
        },
    ],
    audio: [
        {

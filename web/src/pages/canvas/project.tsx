...
import { createDirectorDeskRecord, ensureDirectorDeskRecords, writeDirectorDeskRecords } from "@/pages/director/editor/workspaces/directorDeskRegistry";
...
function InfiniteCanvasPage() {
        ...
        (type: CanvasNodeTypeId, position?: Position) => {
            ...
            const configMetadata =
                type === CanvasNodeType.Config
                    ...
                    : // 每个导演台节点绑定一个新建的导演台实例，节点上保留编号便于下次直达。
                      type === CanvasNodeType.Director
                      ? (() => {
                            const records = ensureDirectorDeskRecords();
                            const record = createDirectorDeskRecord(records);
                            writeDirectorDeskRecords([...records, record]);
                            return { directorDeskId: record.id, directorDeskName: record.name };
                        })()
                      : undefined;
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                ...
                // useBuiltinPanel text 模式 writeBackToSelf:生成结果流式写回插件节点自身(与文本节点同款面板)
                const textBuiltinPanel = sourceNode ? getNodeDefinition(sourceNode.type)?.useBuiltinPanel : undefined;
                if (textBuiltinPanel?.writeBackToSelf && textBuiltinPanel.mode === "text") {
                    let streamed = "";
                    try {
                        const answer = await requestImageQuestion(
                            ...
                            (text) => {
                                streamed = text;
                                setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, content: text } } : node)));
                            },
                            { signal: runController.signal },
                        );
                        const content = answer || streamed;
                        setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, content, status: NODE_STATUS_SUCCESS, errorDetails: undefined } } : node)));
                        setDialogNodeId(null);
                    } catch (error) {
                        if (!isGenerationCanceled(error)) {
                            const errorDetails = error instanceof Error ? error.message : t("canvas.projectPage.generationFailed");
    ...
    const handleOpenDirectorDesk = useCallback(
        (deskId: string, nodeId: string) => {
            navigate(`/director/editor?instanceId=${encodeURIComponent(deskId)}&fromCanvas=${encodeURIComponent(projectId)}&fromNode=${encodeURIComponent(nodeId)}`);
        },
        [navigate, projectId],
    );
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                <CanvasNodeHoverToolbar
                    ...
                    onOpenDirectorDesk={(node) => {
                        const deskId = node.metadata?.directorDeskId;
                        if (deskId) handleOpenDirectorDesk(deskId, node.id);
                    }}
                />
                ...
                <CanvasToolbar
                    ...
                    onAddDirectorDesk={() => createNode(CanvasNodeType.Director)}

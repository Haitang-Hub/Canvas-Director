    ...
    const pluginAi = useMemo<CanvasPluginAi>(() => {
        ...
        return {
            ...
            generateText: async (prompt, options) => {
                ...
                ensureReady(config);
                // references(上游图片)转多模态 content parts,走图片理解通道
                const images = (options?.references || []).filter(Boolean);
                const userContent: AiTextMessage["content"] = images.length ? [{ type: "text" as const, text: prompt }, ...images.map((url) => ({ type: "image_url" as const, image_url: { url } }))] : prompt;

  ...
  return {
    ...
    deleteObject: (id: string) =>
      commitMutation((state) => {
        ...
        const linkedCameraIds = object.kind === "camera" && object.linkedCameraId
          ? new Set([object.linkedCameraId])
          : new Set<string>();
        const nextCameras = linkedCameraIds.size
          ? state.project.cameras.filter((camera) => !linkedCameraIds.has(camera.id))
          : state.project.cameras;
        const nextFocusedCameras = nextCameras.map((camera) =>
          camera.targetObjectId === id
        ...
        const nextActiveCameraId =
          state.project.activeCameraId && linkedCameraIds.has(state.project.activeCameraId)
            ? nextFocusedCameras[0]?.id ?? null
            : state.project.activeCameraId;
        const nextObjects = state.project.objects.filter((item) => item.id !== id);
        const assetsById = new Map(state.project.assets.map((item) => [item.id, item]));
        const remainingAssetRefIds = new Set(
          nextObjects.map((item) => item.assetRefId).filter((assetRefId): assetRefId is string => Boolean(assetRefId))
        );
        ...
        return {
          ...
          selectedObjectIds: state.selectedObjectIds.filter((itemId) => itemId !== id),
          selectedCrowdId: object.crowdId ? null : state.selectedCrowdId,
          directorInspectorMode: "auto",
          project: {
            ...state.project,
            assets: state.project.assets.filter((item) => !removedAssetRefIds.has(item.id)),
    ...
    duplicateObject: (id: string) =>
      commitMutation((state) => {
        ...
        if (object.kind === "camera" && object.linkedCameraId) {
          const cameraIndex = nextCameras.length + 1;
          const nextCameraId = getNextSequentialId(nextCameras.map((item) => item.id), "cam_", cameraIndex);
          const nextObjectId = createObjectIdForDuplicate(nextObjects, object);
          const linkedCamera = nextCameras.find((c) => c.id === object.linkedCameraId);
          if (linkedCamera) {
            nextCameras.push({
              ...cloneJsonValue(linkedCamera),
              id: nextCameraId,
              name: formatSceneItemName("机位", cameraIndex),
              transform: applyOffsetToTransform(linkedCamera.transform, offset),
              target:
                linkedCamera.targetMode === "manual"
                  ? applyPositionOffset(linkedCamera.target, offset)
                  : linkedCamera.target,
              captures: [],
              lastCaptureUrl: null,
            });
            nextObjects.push({
              ...
              id: nextObjectId,
              name: formatSceneItemName("机位", cameraIndex),
              linkedCameraId: nextCameraId,
              transform: nextCameras[nextCameras.length - 1].transform,
            });
          }
          return {
            ...state,
            selectedObjectId: nextObjectId,
            selectedObjectIds: [nextObjectId],
            directorInspectorMode: "auto",
            project: { ...state.project, objects: nextObjects, cameras: nextCameras },
          };
        }

        const nextObjectId = createObjectIdForDuplicate(nextObjects, object);
        const nextCharacterCount =
          object.kind === "character" ? nextObjects.filter((item) => item.kind === "character").length + 1 : null;
        const duplicated: DirectorObject = {
          ...cloneJsonValue(object),
          id: nextObjectId,
          name:
            object.kind === "character" && nextCharacterCount
              ? formatSceneItemName("角色", nextCharacterCount)
              : object.name,
          crowdId: object.crowdId ? getNextCrowdId(nextObjects) : undefined,
          transform: applyOffsetToTransform(object.transform, offset),
        };
        nextObjects.push(duplicated);
        ...
        return {
          ...
          selectedObjectIds: [nextObjectId],
          directorInspectorMode: "auto",
          project: { ...state.project, objects: nextObjects },
        };
      }),

...
import { clearViewportCaptureHandler, setViewportCaptureHandler, type ViewportCaptureRequest } from "../io/captureBridge";
import {
...
function getCssRgbVariable(name: string, fallback: string) {
  ...
  const value = window.getComputedStyle(scopeRoot).getPropertyValue(name).trim();
...
function CanvasCaptureBridge({
  ...
  useEffect(() => {
    ...
    const capture = async ({
      ...
      preset,
      source,
    }: ViewportCaptureRequest): Promise<ScreenshotResult[]> => {
...
export function DirectorCanvas() {
  ...
  const gizmoRightOffset =
    ...
    GIZMO_EDGE_PADDING + (viewportPanelsCollapsed && !motionStudioOpen ? CLOSE_BUTTON_SPACE : 0);

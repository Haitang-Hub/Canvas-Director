...
import { LayoutGrid, LayoutList, LocateFixed, MapPinPlus, Plus, Trash2, Upload } from "lucide-react";
import {
  InspectorAxisGroup,
  InspectorColorField,
  InspectorPanel,
...
export function CharacterPanel() {
  ...
  const [presetLayout, setPresetLayout] = useState<"grid" | "list">("grid");
      ...
      {activeTab === "properties" ? (
        ...
        <InspectorSection title="姿势预设" className="pose-preset-section">
          {role.characterRig && allowsHumanoidActions ? (
            <>
              <div className="preset-layout-bar">
                <span className="preset-layout-label">布局</span>
                <div className="preset-layout-toggle" role="group" aria-label="预设布局方式">
                  <button
                    type="button"
                    aria-pressed={presetLayout === "grid"}
                    onClick={() => setPresetLayout("grid")}
                    className={presetLayout === "grid" ? "is-active" : ""}
                  >
                  ...
                  <button
                    type="button"
                    aria-pressed={presetLayout === "list"}
                    onClick={() => setPresetLayout("list")}
                    className={presetLayout === "list" ? "is-active" : ""}
                  >
                    <LayoutList aria-hidden="true" size={13} />
                    列表
                  </button>
                </div>
              </div>
              <div className={`preset-grid${presetLayout === "list" ? " is-list" : ""}`}>

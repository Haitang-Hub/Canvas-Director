...
async function loadLocalPlugins() {
    ...
    // Uninstall persisted local plugins whose files no longer exist in public/plugins.
    store.plugins.filter((record) => record.local && !urls.includes(record.url)).forEach((record) => uninstallPlugin(record.id));

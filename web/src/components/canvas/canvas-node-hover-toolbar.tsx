...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Plus, RefreshCw, Settings2, Trash2, Ungroup, Upload, Video, Clapperboard } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const canOpenDirectorDesk = node.type === CanvasNodeType.Director && Boolean(node.metadata?.directorDeskId);
    ...
    const baseToolbarTools: ToolbarTool[] = [
        ...
        ...(canOpenDirectorDesk && onOpenDirectorDesk ? [{ id: "openDirectorDesk", title: t("canvas.nodeToolbar.openDirectorDesk"), label: t("canvas.nodeToolbar.openDirectorDesk"), icon: <Clapperboard className="size-4" />, onClick: () => onOpenDirectorDesk(node) }] : []),
    ];

...
function DirectorNodeContent({ node, theme }: NodeContentRendererProps) {
    const { t } = useTranslation();
    const deskName = node.metadata?.directorDeskName || node.title || t("canvas.nodeTypes.director");

    return (
        <div className="pointer-events-none flex h-full w-full flex-col items-center justify-center gap-2 p-4 text-center">
            <span className="grid size-11 shrink-0 place-items-center rounded-xl" style={{ background: theme.node.fill, border: `1px solid ${theme.node.stroke}`, color: "#ec4899" }}>
                <Clapperboard className="size-6" strokeWidth={1.6} />
            </span>
            <span className="max-w-full truncate text-sm font-semibold" style={{ color: theme.node.text }}>{deskName}</span>
            {node.metadata?.directorDeskId ? (
                <span className="max-w-full truncate font-mono text-[11px]" style={{ color: theme.node.muted }}>{node.metadata.directorDeskId}</span>
            ) : null}
            <span className="text-[11px]" style={{ color: theme.node.muted }}>{t("canvas.node.openDirectorDeskHint")}</span>
        </div>
    );
}
